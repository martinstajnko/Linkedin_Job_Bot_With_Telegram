...
class JobSearchScraper(BaseScraper):
    ...
    async def search(
        ...
        Args:
            ...
            remote: If True, filter to remote positions only (f_WT=2)
            posted_within_hours: Restrict results to jobs posted within this
                many hours. Common values: 1, 24, 168 (1 week).

        Returns:
            List of job posting URLs
        """
        logger.info(
            f"Starting job search: keywords='{keywords}', location='{location}', "
            f"remote={remote}, posted_within_hours={posted_within_hours}"
        )

        search_url = self._build_search_url(keywords, location, remote, posted_within_hours)
    ...
    def _build_search_url(
        ...
        LinkedIn filter params used:
            f_WT=2              → remote only
            f_TPR=r<seconds>    → posted within N seconds
        """
